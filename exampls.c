...
static PyObject* Mesh_getname(Mesh *self) {
    return PyUnicode_FromString(self->ptr->mName.data);
}

static PyObject* Mesh_get_numverts(Mesh *self) {
    return PyLong_FromUnsignedLong(self->ptr->mNumVertices);
}

static PyObject* Mesh_getverts(Mesh *self) {
    size_t nverts = self->ptr->mNumVertices;
    PyObject *verts = PyList_New(nverts);
    for(uint i = 0; i < nverts; i++) {
        struct aiVector3D tmp = self->ptr->mVertices[i];
        PyObject *args = Py_BuildValue("ddd", tmp.x, tmp.y, tmp.z);
        PyObject *v = PyObject_CallObject((PyObject*)&Vec3DType, args);
...
static PyGetSetDef Mesh_getsetters[] = {
    {"name", (getter)Mesh_getname, NULL, NULL},
    {"num_verts", (getter)Mesh_get_numverts, NULL, NULL},
    {"verts", (getter)Mesh_getverts, NULL, NULL},
    {NULL}
};
...
static int Scene_init(Scene *self, PyObject *args, PyObject *kwds) {
    ...
    self->ptr = (struct aiScene*)aiImportFile(filepath, flags);
    if (!self->ptr) {
...
static PyObject* Scene_get_nummeshes(Scene *self, void* closure) {
    return PyLong_FromLong(self->ptr->mNumMeshes);
}

static PyObject* Scene_get_meshes(Scene *self, void* closure) {
    size_t num_meshes = self->ptr->mNumMeshes;

    PyObject *meshes = PyList_New(num_meshes);
    for (uint i = 0; i < num_meshes; i++) {
        Mesh *m = (Mesh*)(MeshType.tp_alloc(&MeshType, 0));
        Py_INCREF(m);
        m->ptr = self->ptr->mMeshes[i];
        PyList_SetItem(meshes, i, (PyObject*)m);
    }
    return meshes;
}

static PyObject* Scene_get_nummaterials(Scene *self, void* closure) {
    return PyLong_FromLong(self->ptr->mNumMaterials);
}

static PyGetSetDef Scene_getsetters[] = {
    {"meshes", (getter)Scene_get_meshes, NULL, NULL},
    {"num_meshes", (getter)Scene_get_nummeshes, NULL, NULL},
    {"num_materials", (getter)Scene_get_nummaterials, NULL, NULL},
    {NULL}
};
...
static PyObject* ImportFile(PyObject *self, PyObject *args) {
    PyObject *pyscene = PyObject_CallObject((PyObject*)&SceneType, args);
    return pyscene;
}
